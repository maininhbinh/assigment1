window.BookingNomarControllers = function($scope, $routeParams, $http){
    $scope.idNomar = Number($routeParams.id);
    const apiKhoaHocNomar = "http://localhost:3000/khoaHocNomar";
    
    $scope.getDataKhoaHocNomar = function() {
        $http.get(`${apiKhoaHocNomar}/${$scope.idNomar}`).then(function(res){
            if(res.status == 200){       
